...
use super::{
  ...
  user_service::BinanceUserService,
...
pub async fn subscribe_websocket(
  ...
  let connection_id = match request.stream_type {
      StreamType::Ticker => {
          service.subscribe_ticker(&request.symbol).await
      },
      StreamType::Kline => {
      ...
      StreamType::Depth => {
          service.subscribe_depth(&request.symbol).await
      },
  };
  ...
  let result = match request.type_ {
      OrderType::Limit => {
          let price = request.price.ok_or_else(|| {
              (
                  StatusCode::BAD_REQUEST,
                  Json(ErrorResponse { error: "Price is required for limit orders".to_string() }),
              )
          })?;
          
          match request.side {
              OrderSide::Buy => service.limit_buy(&request.symbol, request.quantity, price).await,
              OrderSide::Sell => service.limit_sell(&request.symbol, request.quantity, price).await,
          }
      },
      OrderType::Market => {
          match request.side {
