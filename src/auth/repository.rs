...
impl AuthRepository {
    ...
    pub async fn find_user_by_email(&self, email: &str) -> Result<Option<User>, AppError> {
        ...
        match user_doc {
            Some(doc) => {
                let user: User = bson::from_document(doc).map_err(|e| {
                    AppError::DatabaseError(format!("Failed to deserialize user: {}", e))
                })?;
                Ok(Some(user))
            }
            None => Ok(None),
    ...
    pub async fn store_telegram_token(
        &self,
        user_id: &str,
        token: &str,
    ...
    pub async fn get_telegram_token(
        &self,
        token: &str,
    ) -> Result<Option<(String, chrono::DateTime<chrono::Utc>)>, AppError> {
        let result = self
            .db
            .collection("auth_tokens")
            .find_one(doc! { "token": token, "type": "telegram" }, None)
            .await
            .map_err(|e| AppError::DatabaseError(format!("Failed to query token: {}", e)))?;
        ...
        match result {
            Some(doc) => {
                ...
                let expiration = doc.get_datetime("expiration").map_err(|e| {
                    AppError::InternalError(format!("Failed to parse expiration: {}", e))
                })?;

                Ok(Some((user_id.to_string(), expiration.to_chrono())))
            }
            None => Ok(None),
        }
    }

    pub async fn delete_telegram_token(&self, token: &str) -> Result<(), AppError> {
        self.db
            .collection("auth_tokens")
            .delete_one(doc! { "token": token, "type": "telegram" }, None)
            .await
